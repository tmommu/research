# -*- coding: utf-8 -*-
"""TIBOK — Model Quantization & Testing (Section G), MIT-BIH only

Matches RIM-01_Pre-Oral_9.pdf Section G: post-training INT8 full-integer
quantization of the FP32 network trained by `mitbih_train.py` (Section C),
and the in-silico FP32-vs-INT8 validation protocol described in "Scopes and
Limitations" — benchmarking the INT8-quantized embedded network against the
FP32 desktop baseline on the same held-out test set, across F1-score,
sensitivity, specificity, and the other Section XII metrics.

Requires `mitbih_train.py` to have been run first (needs
`<RUN_TAG>_model.keras` and `<RUN_TAG>_train_summary.json` in OUTPUT_DIR).
Re-derives the validation/test arrays via `mitbih_data.load_all()` — this is
deterministic (fixed seed=42), so it reproduces byte-for-byte the same
splits and RR normalization the training stage used; a saved-vs-recomputed
rr_mean/rr_std check below guards against silent drift (e.g. a different
machine's PhysioNet mirror serving a different record cut).
...
if not os.path.exists(MODEL_PATH) or not os.path.exists(TRAIN_SUMMARY_PATH):
    raise FileNotFoundError(
        f"Missing {MODEL_PATH} or {TRAIN_SUMMARY_PATH}. Run mitbih_train.py "
        "(Section C) first — this script picks up where it left off."
    )
...
# ---------------------------------------------------------------------------
# Re-derive validation/test data (deterministic — see mitbih_data.py)
# ---------------------------------------------------------------------------
...
if not (np.allclose(rr_mean, saved_mean) and np.allclose(rr_std, saved_std)):
    raise RuntimeError(
        ...
        "until this is resolved (re-run mitbih_train.py to retrain, or "
...
# ---------------------------------------------------------------------------
# Per-AAMI-symbol sensitivity breakdown (INT8, at the recommended deploy
# threshold), to compare directly against the FP32 breakdown saved by
# mitbih_train.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Save quantize_test_summary.json
# ---------------------------------------------------------------------------
