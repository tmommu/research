# -*- coding: utf-8 -*-
"""TIBOK — Combined MIT-BIH + INCART, RR-Interval-Fused 1D-CNN (standalone, EXTENSION)

**This is NOT the protocol described in RIM-01_Pre-Oral_9.** The plan's
Section C ("Software: Model Architecture Training"), Section XI ("Data
Collection"), Section XII ("Data Analysis"), and "Scopes and Limitations"
all describe training/validating on **MIT-BIH only**; INCART / St. Petersburg
is never mentioned anywhere in that document. For the run that actually
matches the plan as written, use `mitbih_only_validation.py` in this same
directory — that is the one to cite as "the Model Validation Testing step
outlined in the plan."

This script is kept alongside it as a labeled ablation/extension (pooling in
INCART's more varied population is a standard fix for the inter-patient
generalization gap that MIT-BIH-only training leaves open — Qi et al., 2023),
in case the combined-database protocol is something you want to present in
addition to the plan's baseline. If you do want to present these numbers,
the pre-oral document needs a corresponding update (citing INCART, its
sample rate/lead handling, and the resulting split) before the defense —
right now a panelist reading Section C/XI/XII would have no idea this
database was involved. See model_validation/README.md.

Trains on the **combined MIT-BIH + St. Petersburg INCART Arrhythmia Databases**,
patient-level 70% train / 15% val / 15% test split. INCART's 75 twelve-lead
recordings are resampled from their native 257 Hz to the shared 360 Hz working
rate, with lead II pulled as the MLII-equivalent channel per the plan's stated
data-collection approach.

Each ECG window is paired with 4 RR-interval features (pre-RR, post-RR, causal
local-average RR, prematurity ratio), fused into the classifier head next to
the CNN's pooled output. Pooling MIT-BIH with INCART's more varied population
is the standard fix for the inter-patient generalization gap that MIT-BIH-only
training leaves open (Qi et al., 2023).
...
Unlike the Colab notebook this was exported from, this script pulls MIT-BIH
and INCART directly from PhysioNet into a local cache directory (no Google
Drive mount required) and saves all artifacts to disk instead of triggering a
browser download. See model_validation/README.md for setup and how this maps
to the plan.
"""
...
#
# The pooled 119 "patients" are shuffled once (fixed seed) and split 70/15/15
# — train and test therefore each contain a mix of both databases, so this
# protocol is not directly comparable to the classic de Chazal DS1/DS2
# MIT-BIH-only benchmark table; it's the protocol the plan's methodology
# commits to, evaluated on a broader population.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Local cache for MIT-BIH + INCART, downloaded directly from PhysioNet
#
# Both databases are fetched into DATA_ROOT the first time this script runs
# and re-used on subsequent runs. Override DATA_ROOT via the TIBOK_DATA_ROOT
# environment variable if you want the cache to live somewhere other than
...
INCART_PATH = os.path.join(DATA_ROOT, "incartdb")
...
_ensure_database('incartdb', INCART_PATH)

# Recursively index every .hea file under each path, so it doesn't matter
# whether files sit directly in the folder or inside a nested subfolder.
...
# ---------------------------------------------------------------------------
# Loading + RR-interval feature extraction (both databases)
#
# MIT-BIH loads at its native 360 Hz directly from MITDB_PATH; INCART loads
# from INCART_PATH and is resampled from 257 Hz to 360 Hz
# (scipy.signal.resample), with its annotation sample indices rescaled to
# match, so every downstream step (windowing, RR-interval computation in
# seconds) operates in one consistent 360 Hz time base regardless of source.
# INCART's lead II is used as the MLII-equivalent channel, per the plan's
# stated data-collection approach.
# ---------------------------------------------------------------------------
...
#
# Time shift, baseline wander, powerline hum, Gaussian noise, amplitude
# scaling — applied 2x to arrhythmia-class windows only. RR features are
# carried over unchanged for augmented copies since the augmentation only
# perturbs the signal, not beat timing.
# ---------------------------------------------------------------------------
...
# the classifier head. Fixed (both-term) focal loss plus a manually boosted
# positive-class weight — this is a screening device, so a false negative
# (missed arrhythmia) should cost more than a false positive.
# ---------------------------------------------------------------------------
...
#
# Reports three operating points: F1-optimal, Youden's-J (balanced
# sensitivity/specificity), and a high-sensitivity point targeting >=90%
# recall, plus a precision-floor point. Given TIBOK is a *screening* device
# — the hypotheses explicitly treat a missed arrhythmia as costlier than a
# false alarm — the Youden or high-sensitivity threshold is the more
# defensible deployment choice, not F1-optimal.
# ---------------------------------------------------------------------------
...
# reaches >=90% precision there (picking the lowest such threshold keeps
# recall as high as possible while satisfying the floor). Chosen on val,
# only ever evaluated on test — same no-leakage protocol as the other three
# thresholds. Re-check this after every retrain, since the "best" threshold
# depends on that run's specific precision-recall curve.
...
#
# The pooled "arrhythmia" sensitivity can hide a real weakness — if the model
# is strong on common V-beats but weak on rarer F or J beats, the pooled
# number still looks fine because V-beats dominate the positive class.
# ---------------------------------------------------------------------------
...
# nRF52840
#
# Calibrates on the validation set (never the test set), converts both
# inputs to INT8, and re-evaluates on the same held-out test set to answer
# whether quantization changes F1 / specificity, per Section G and the
# in-silico FP32-vs-INT8 validation protocol in Scopes and Limitations.
#
...
# fails here, the cell below automatically falls back to dynamic-range
# (weight-only INT8) quantization, which still shrinks the model ~4x and
# runs correctly on the nRF52840's Cortex-M4 FPU.
# ---------------------------------------------------------------------------
