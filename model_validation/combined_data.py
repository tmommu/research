# -*- coding: utf-8 -*-
"""Shared MIT-BIH+INCART data pipeline for combined_train.py and
combined_quantize_test.py.

**Extension, not the plan's protocol** — see README.md. Deterministic (fixed
seed=42 throughout): re-running load_all() reproduces the exact same
train/val/test split, arrays, and RR-feature normalization every time, so
the quantization/testing stage can independently regenerate the same
validation and test sets the training stage used, without needing to
serialize the raw arrays to disk.
"""
...
def get_splits():
    """Patient-level 70/15/15 split over the pooled 119 records. Fixed seed=42."""
...
def load_and_segment(source, record_id, window_size, mitdb_index, incart_index):
    if source == 'mitdb':
...
def build_dataset(record_list, mitdb_index, incart_index):
    all_segments, all_labels, all_symbols, all_rr = [], [], [], []
    for source, rid in record_list:
        segs, labs, syms, rrf = load_and_segment(source, rid, WINDOW_SIZE, mitdb_index, incart_index)
...
def load_all():
    """Downloads (if needed), loads, and preprocesses MIT-BIH+INCART into arrays.
    ...
    print(f"Total patient-records: {len(ALL_RECORDS)} (MIT-BIH {len(MITDB_RECORDS)} + INCART {len(INCART_RECORDS)})")
    ...
    train_segments, train_labels, train_symbols, train_rr = build_dataset(train_records, mitdb_index, incart_index)
    print("\nLoading validation records...")
    val_segments, val_labels, val_symbols, val_rr = build_dataset(val_records, mitdb_index, incart_index)
    print("\nLoading held-out test records...")
    test_segments, test_labels, test_symbols, test_rr = build_dataset(test_records, mitdb_index, incart_index)
