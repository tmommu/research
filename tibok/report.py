...
def summary_rows(rows, group_prefixes=("FP32", "INT8"), nd=4):
    ...
    for prefix in group_prefixes:
        sel = [r for r in rows if prefix in str(r["Model"]) and "mean" not in str(r["Model"])]
        if not sel:
            continue
        for label in dict.fromkeys(r["Threshold Type"] for r in sel):
            ...
            for stat in ("mean", "sd"):
                if stat == "sd" and n < 2:
                    continue
                row = {"Model": f"{prefix} {stat} (n={n})", "Threshold Type": label}
...
def build_results_table(candidates, val_pr_aucs, selected_index, thresholds,
    ...
    for i, model in enumerate(candidates):
        mark = " [Selected]" if i == selected_index else ""
        label = f"C{i + 1}{mark}"

        probs = model.predict([X_test, RR_test_n], batch_size=256, verbose=0).flatten()
        rows += result_rows(f"{label} FP32", y_test, probs, thresholds, nd=nd)
        ...
        if quantize:
            try:
                ...
                rows += result_rows(f"{label} INT8", y_test, q_probs, thresholds, nd=nd)
            except Exception as e:
                print(f"  {label}: INT8 conversion failed, FP32 row only -- {type(e).__name__}: {e}")

        pa = val_pr_aucs[i] if i < len(val_pr_aucs) else float("nan")
        print(f"  {label:<16} val PR-AUC {pa:.4f}")
